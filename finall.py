    ...
    if hands:
        ...
        for button in buttonList:
            ...
            if x < lmList[8][0] < x + w and y < lmList[8][1] < y + h:
                cv2.rectangle(img, (x - 5, y - 5), (x + w + 5, y + h + 5), highlight_color, cv2.FILLED)
                cv2.putText(img, button.text, (x + 20, y + 65),
                            cv2.FONT_HERSHEY_PLAIN, 4, text_color, 4)
                ...
                if l < 50:  # Adjust the distance threshold for click
                    ...
                    cv2.circle(img, (lmList[8][0], lmList[8][1]), 10, click_color, cv2.FILLED)
                    
    # Display final text with custom background color
    cv2.rectangle(img, (50, 350), (700, 450), background_color, cv2.FILLED)
